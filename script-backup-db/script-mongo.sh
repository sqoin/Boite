...
if [ ${DATABASE_NAMES} = "database-name" ]; then
        ...
        do
                mongodump --host ${MONGO_HOST} --port ${MONGO_PORT} --db ${DB_NAME}  --out ${DB_BACKUP_PATH}/${TODAY}/
        done
fi
...
cd ~/${DB_BACKUP_PATH}
  today= tar -czvf ${TODAY}.tar.gz ${TODAY}
  todayzip=${TODAY}.tar.gz


 echo "Hi, Your database backup for date backup mongodb tokenmarketcap is ready" | mutt -a /root/${DB_BACKUP_PATH}/$todayzip -s "Database Backup - $todayzip" -- address-email 
######################### End of script ############################
